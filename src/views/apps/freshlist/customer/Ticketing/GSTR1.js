...
class GSTR1 extends React.Component {
    ...
    this.state = {
      ...
      HSN: [
          ...
          cellRendererFramework: (params) => {
            return (
              <div>
                <span>
                  {(
                    (params?.data?.Product_MRP * params?.data?.GSTRate) /
                    2 /
                    100
                  )?.toFixed(2)}
                </span>
        ...
        },
        {
          ...
          cellRendererFramework: (params) => {
            let amount =
              params?.data?.Product_MRP * ((100 + params?.data?.GSTRate) / 100);
            return (
              <div>
  ...
  toggleTab = (tab) => {
    ...
    if (tab == 1) {
      ...
      this.setState({ columnDefs: [...B2B] });
    } else if (tab == 2) {
      this.setState({ Dropdown: false });
      this.setState({ rowData: this.state.rowAllData });
      this.setState({ columnDefs: [...B2CL] });
    } else if (tab == 3) {
      this.setState({ Dropdown: false });
      this.setState({ rowData: this.state.rowAllData });
      this.setState({ columnDefs: [...B2CS] });
    } else if (tab == 4) {
      this.setState({ Dropdown: false });
      this.setState({ rowData: this.state.rowAllData });
      this.setState({ columnDefs: [...CDNR] });
      let A = this.state.CreditNote?.filter(
        (ele) => ele?.partyId?.Registration_Type == "Regular"
      );
      this.setState({ rowData: A ? A : [] });
    } else if (tab == 5) {
      // start
      this.setState({ Dropdown: false });
      this.setState({ columnDefs: [...CDNUR] });
      let A = this.state.CreditNote?.filter(
        (ele) => ele?.partyId?.Registration_Type !== "Regular"
      );
      this.setState({ rowData: A ? A : [] });
      ...
      // this.setState({ rowData: this.state.HSNSummary });
    }
  };
  toggleModal = () => {
    ...
    await CreditnoteOrderList(id, db)
      .then((res) => {
        ...
        if (res?.CreditNote?.length) {
          this.setState({ CreditNote: res?.CreditNote });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });
