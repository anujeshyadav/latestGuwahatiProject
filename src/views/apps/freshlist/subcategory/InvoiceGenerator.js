...
class InvoiceGenerator extends React.Component {
  ...
  MergeBillNow = async (data) => {
    ...
    await _Get(View_Customer_ById, data?.partyId?._id)
      .then((res) => {
        console.log(res);
        let Transporter = res?.Customer[0].assignTransporter?.map((ele) => {
          return ele?.id;
        });
        this.setState({
  ...
  handleAddCharges = (e) => {
    ...
    if (value?.igstTaxType == 1) {
      ...
      value["cgstTotal"] = value?.cgstTotal + othergst;
      value["sgstTotal"] = value?.sgstTotal + othergst;
      value["grandTotal"] = Number(
    ...
    let decimalValue;
    const containsDecimal = /\./.test(value?.grandTotal);
    // let DecimalStatus = value?.grandTotal.includes(".");
    if (containsDecimal) {
      decimalValue = Number(value?.grandTotal?.toString()?.split(".")[1]);
      if (decimalValue > 65) {
        let roundoff = 100 - decimalValue;
        value["grandTotal"] = parseFloat(value.grandTotal) + roundoff / 100;
        // value["grandTotal"] = value?.grandTotal + roundoff / 100;
        console.log(value);
        invoiceData["roundOff"] = +roundoff;
      } else {
        let roundoff = -decimalValue;
        value["grandTotal"] = parseFloat(value.grandTotal) - decimalValue / 100;
        console.log(value);
        value.roundOff = roundoff / 100;
      }
    } else {
      // decimalValue = Number(value?.grandTotal);
                            ...
                            <Row className="main div heading px-3 py-3">
                              <Col lg="6">
                                ...
                                <Input
                                  ...
                                  value={this.state.PrintData?.amount?.toFixed(
                                    2
                                  )}
                              ...
                              <Col lg="6">
                                <Label className="mb-1">Other Charges</Label>

                                <CustomInput
                                  value={this.state.PrintData?.otherCharges}
                                  onChange={this.handleAddCharges}
                                  type="select">
                                  <option value={0}>--select--</option>
                                  {this.state.OtherCharges?.length > 0 &&
                                    this.state.OtherCharges?.map((ele, i) => {
                                      return (
                                        <option
                                          data_id={`${ele?.amount}*${ele?.percentage}`}
                                          value={ele?.amount}>
                                          {ele?.title}
                                        </option>
                                      );
                        ...
                        this.state.ViewOneData?.orderItems?.map((ele, i) => (
                          <>
                            <tr>
                              ...
                              <td>{ele?.unitType}</td>
                              <td>{ele?.qty}</td>
                              <td>{this.state.ViewOneData?.grandTotal}</td>
                            </tr>
                          </>
                        ))}
                    </tbody>
